...
@PostMapping("/api/loans/manager")
    ...
    public ResponseEntity<Object> newLoan(Authentication authentication
        ...
        if ( loan.isEmpty() ){
            ...
            clientLoans =  client.getClientLoans().stream().filter(clientLoan -> clientLoan.getLoan().getName().equalsIgnoreCase(loan.get().getName()) && clientLoan.getAmount() > 0).collect(Collectors.toSet());
        }
    ...
    public ResponseEntity<Object> PayLoans (Authentication authentication ,
        ...
        if ( amountEntered < clientLoan.getAmount() ){
